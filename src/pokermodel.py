...
class Player(QObject):
    def __init__(self, name, starting_money):
        ...
        self.money = MoneyModel(
            starting_money
        )  # Define the amount of money the players start with
...
class TexasHoldEm(QObject):
    ...
    active_player_changed = (
        pyqtSignal()
    )  # Signal only handling when the active player is changed.
    ...
    def check(self):
        ...
        if diff != 0:
            ...
            ):  # After both players checks the first time the flop is dealt
                self.deal(3)
            elif (
                self.check_counter == 4 or self.check_counter == 6
            ):  # After both check again another card is dealt
                self.deal(1)
            elif (
                self.check_counter == 8
            ):  # When all 5 cards is on the table and both players check, winner is checked.
    ...
    def bet(self, amount: int):
        if self.players[self.active_player].money.value <= 0:
            ...
            max_bet = max([player.betted.value for player in self.players])
            minimum_allowed_bet = (
                max_bet - self.players[self.active_player].betted.value
            )
    ...
    def call(self):
        ...
        if (
            amount != 0
        ):  # Proceeds if a call is possible otherwise a message is provided.
            self.pot += amount
            self.players[self.active_player].place_bet(amount)
            self.change_active_player()
        else:
    ...
    def fold(self):
        ...
        self.game_message.emit(
            self.players[self.active_player].name + " won $ " + str(self.pot.value)
        )
        self.__new_round()
        self.players[self.active_player].hand.flip()
    ...
    def check_round_winner(self):
        best_poker_hands = [
            player.hand.best_poker_hand(self.table.cards) for player in self.players
        ]  # Saves both
        ...
        if best_poker_hands[0] > best_poker_hands[1]:
            ...
            self.game_message.emit(
                self.players[0].name + " won $ " + str(self.pot.value)
            )

        elif best_poker_hands[1] > best_poker_hands[0]:
            self.players[1].receive_pot(self.pot.value)
            self.game_message.emit(
                self.players[1].name + " won $ " + str(self.pot.value)
            )
